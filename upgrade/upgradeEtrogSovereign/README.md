...
4. **Tokens file**
   You'll need the path to a `tokens.json` file containing the list of wrapped tokens and `initNativeSupply` at block 0 (only ETH) for bridge contract in order to run the script and perform the upgrade.
   This JSON file must have the following structure (generated by `tools/getLBT/getLBT.ts` with `printTokens: true`):
